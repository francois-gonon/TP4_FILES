# PowerShell script to set up Airflow connections for MinIO and other services

Write-Host "Setting up Airflow connections..." -ForegroundColor Green

# MinIO S3 Connection
Write-Host "Creating MinIO S3 connection..." -ForegroundColor Yellow
...
Write-Host "Creating PostgreSQL connection..." -ForegroundColor Yellow
...
Write-Host "Creating AWS S3 source connection..." -ForegroundColor Yellow
docker exec airflow-webserver airflow connections add 'aws_s3_source' `
    --conn-type 'aws' `
    --conn-extra '{\"region_name\": \"us-east-1\"}'

Write-Host "Airflow connections setup completed!" -ForegroundColor Green

# List all connections to verify
Write-Host "Current Airflow connections:" -ForegroundColor Cyan
