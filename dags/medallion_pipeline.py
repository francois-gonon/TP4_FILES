...
def run_transformation_stage(stage_name):
    """Run a specific transformation stage"""
    try:
        # Import our pandas transformation functions
        ...
        if stage_name == "landing_to_bronze":
            return landing_to_bronze_transformation()
        elif stage_name == "bronze_to_silver":
            return bronze_to_silver_transformation()
        elif stage_name == "silver_to_gold":
            return silver_to_gold_transformation()
        else:
...
landing_to_bronze_task = PythonOperator(
    ...
    python_callable=run_transformation_stage,
    op_args=['landing_to_bronze'],
    dag=dag,
)
